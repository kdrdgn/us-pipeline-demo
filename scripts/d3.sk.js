...
sk.createSankey = function(containerId, configSankey, dataSankey) {
  ...
  var canvas_base = d3.select('canvas');
  if( !canvas_base.size() ){
    canvas_base = container.append("canvas")
      ...
      .attr("class", "sk-canvas canvas-hidden");
  }
    ...
    var link = svg.append("g").selectAll(".sk-link")
        ...
        .attr("class", "sk-link")
        .attr("ltype", function(l){ return l.value > Number.MIN_VALUE ? l.ltype : ''; })
        .attr("d", path)
  ...
  var pipelineNames = svg.append("text").attr("class", "sk-pipeline").selectAll(".sk-link")
    .data(_dataSankey.links, function(l) {
      return l.name;
    })
    .enter().append("textPath")
    .attr("href", function(pipeline){
        return '#' + pipeline.id
    })
    .attr("dy", ".35em")
    ...
    .text(function(d) {
        if(d.name != '' && d.value > Number.MIN_VALUE){
            return d.name + ' - ' + _formatValueTooltip(d.overload ? d.overload : d.value);
        }
      return '';
    })
    .filter(function(d) {
  ...
  var nomLinks = _dataSankey.links.filter( l => l.ltype === 'nomination' && l.value > Number.MIN_VALUE );

  var linkExtent = d3.extent(nomLinks, function (d) {return d.value});
  ...
  function tick(elapsed, time) {
    ...
    d3.selectAll('path.sk-link[ltype="nomination"]')
      .each(
        function (d) {
//        if (d.freq < 1) {
          for (var x = 0;x<2;x++) {
            var offset = (Math.random() - .5) * (d.y0 - 2);
            if (Math.random() < d.freq) {
              particles.push({link: d, time: elapsed, offset: offset, path: this, animateTime: length, speed: 0.5 + (Math.random())})
            }
          }
...
//        }
  ...
  function particleEdgeCanvasPath(elapsed) {
    ...
    var canvasWidth = dimensions.width + configSankey.margin.left + configSankey.margin.right;
    var canvasHeight = dimensions.height + configSankey.margin.top + configSankey.margin.bottom;
...
//        var currentPercent = currentTime / 1000 * particles[x].path.getTotalLength();
      ...
      context.arc(currentPos.x + 40 , currentPos.y + particles[x].offset, particles[x].link.particleSize, 0, 2 * Math.PI);
      context.fill();
    }
    ...
    var _updateLinksValues = function(dataUpdated) {
        console.log('_updateLinksValues', dataUpdated);
        _stopTooltips();
        sankey(dataUpdated);
        ...
        svg.selectAll(".sk-link")
            ...
            .transition()
            .duration(1300)
            .attr("ltype", function(d){ return d.value > Number.MIN_VALUE ? d.ltype : ''; })
            .attr("d", path)
            .style("stroke", function(l) {
              return l.color;
            })
            ...
            svg.selectAll(".sk-pipeline textPath")
            .data(dataUpdated.links, function(d) {
                return d.id;
            }).text(function(d) {
              if(d.name != '' && d.value > Number.MIN_VALUE){
      ...
      nomLinks = _dataSankey.links.filter( l => l.ltype === 'nomination' && l.value > Number.MIN_VALUE );

      linkExtent = d3.extent(nomLinks, function (d) {return d.value});
    ...
    this.updateData = function(dataUpdated) {
        for (var i = 0; i < dataUpdated.links.length; i++) {
            ...
            if (linkToUpdate){
                linkToUpdate.value = _safeValueToLink(dataUpdated.links[i].value);
                linkToUpdate.overload = dataUpdated.links[i].overload;
                linkToUpdate.name = dataUpdated.links[i].name;
                if( dataUpdated.links[i].color ){
                    linkToUpdate.color = dataUpdated.links[i].color;
                }
            } 
        }
